...
var LedgerVap = function(comm) {
	this.comm = comm;
	this.comm.setScrambleKey('w0w');
}

LedgerVap.prototype.getAddress_async = function(path, boolDisplay, boolChaincode) {
...
LedgerVap.prototype.signTransaction_async = function(path, rawTxHex) {
	var splitPath = utils.splitPath(path);
	var offset = 0;
...
LedgerVap.prototype.getAppConfiguration_async = function() {
...
LedgerVap.prototype.signPersonalMessage_async = function(path, messageHex) {
	var splitPath = utils.splitPath(path);
	var offset = 0;
